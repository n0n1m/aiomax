...
- `participants: ¯\_(ツ)_/¯ | None` - документация Max говорит, что это "участники чата с временем последней активности". как именно объект выглядит не сказано. Но может быть `None` если запрашивается список чатов
