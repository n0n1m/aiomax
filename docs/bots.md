...
`Bot` по сути является главным роутером. Подробнее о роутерах в [routers.md](routers.md).
...
- `case_sensitive: bool` - важнен ли регистр у команд. Например, если `False`, то команду с названием `test` можно будет ввести как и через `/test`, так и через `/TEST`. `True` по умолчанию
