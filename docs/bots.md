...
Класс `Router` является суперклассом `Bot`, так что `Bot` поддерживает все методы из `Router`.
...
- `case_sensitive: bool` - важна ли капитализация текста у команд. Например, если `False`, то команду с названием `test` можно будет ввести как и через `/test`, так и через `/TEST`. `True` по умолчанию (этот аргумент также доступен в `aiomax.Router`)
